...
public class PercentProgressBar extends View {
    ...
    //Horizontal状态的默认高
    private final float default_layout_height;
    //外部圆环的默认宽度
    private final float default_outer_width;
    //外部圆环的默认颜色
    private final int default_outer_color;
    //外部圆环的默认透明度
    private final int default_outer_alpha;
    //ProgressBar的默认宽度
    private final float default_progress_bar_width;
    //ProgressBar的默认颜色
    ...
    public PercentProgressBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        //初始化默认参数
        default_layout_radius = dp2px(150);
        default_layout_width = dp2px(350);
        default_layout_height = dp2px(30);
        default_outer_width = dp2px(15);
        default_outer_color = 0XFFFFFF;
        default_outer_alpha = 50;
        default_progress_bar_width = dp2px(10);
        ...
        mTextSize = ta.getDimension(R.styleable.PercentProgressBar_text_size, default_text_size);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST){
            if (mProgressBarStyle == PROGRESS_BAR_CIRCULAR){
                width = (int) default_layout_radius;
            } else {
                width = (int) default_layout_width;
            }
        } else {
            width = widthSpecSize;
        }
        if (heightSpecMode == MeasureSpec.AT_MOST){
            if (mProgressBarStyle == PROGRESS_BAR_CIRCULAR){
                height = (int) default_layout_radius;
            } else {
                height = (int) default_layout_height;
            }
        } else {
    ...
    private void calculateProgressBarPath() {
        int offset = 30;
        percent = (mCurrentProgress / (float)mMaxProgress);
    ...
    private void drawText(Canvas canvas) {
        float textWidth = mTextPaint.measureText((int)(percent * 100) + "%");
        ...
        canvas.drawText((int)(percent * 100) + "%", x, y, mTextPaint);
    }
    ...
    private void drawLittleCircle(Canvas canvas) {
        ...
        if (mProgressBarStyle == PROGRESS_BAR_CIRCULAR) {
            ...
            canvas.translate(paddingLeft + mTextWidth + offset, (paddingTop + height + paddingBottom) / 2);
        }
